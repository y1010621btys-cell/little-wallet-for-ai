...
@mcp.tool()
def taobao_search(keyword: str) -> str:
    """淘宝搜索商品。返回前几条的 id / 标题 / 价格 / 图片，供挑选。
    海外 IP 渲染偏慢，单次可能要等 ~30 秒。"""
    try:
        return json.dumps(tb_buy.cmd_search(keyword), ensure_ascii=False)
    except Exception as e:
        return json.dumps({"error": str(e)}, ensure_ascii=False)


@mcp.tool()
def taobao_add_cart(item_id: str, quantity: int = 1) -> str:
    """把商品加入购物车（绕开详情页验证码）。item_id 从 taobao_search 拿。"""
    try:
        return json.dumps({"ok": tb_buy.cmd_add(item_id, quantity)})
    except Exception as e:
        return json.dumps({"ok": False, "error": str(e)}, ensure_ascii=False)


@mcp.tool()
def taobao_buy(item_id: str, title: str = "", price: Optional[float] = None,
               img: str = "") -> str:
    """加购 + 递一张钱包申请条，让人在淘宝 App 里选规格、付款。
    机器不付款；price 是估价，付款时可按实际调（见条子的 final_price）。"""
    try:
        resp = tb_buy.cmd_buy(item_id, title=title or None, price=price,
                              img=img or None)
